...
// --- QUAN TRỌNG: IMPORT CANNONBALL VÀ PROJECTILE ---
...
public class ObjectManager {
    ...
    private BufferedImage spikeImg; // Bỏ cannonBallImg ở đây vì class CannonBall tự load
    private ArrayList<Potion> potions;
    private ArrayList<GameContainer> containers;
    
    // --- SỬA: Dùng danh sách CannonBall ---
    private ArrayList<CannonBall> projectiles = new ArrayList<>();
	...
	public void checkObjectHit(ArrayList<Arrow> arrows) {
        for (Arrow a : arrows) {
            if (a.isActive()) {
                for (GameContainer gc : containers) {
                    if (gc.isActive() && !gc.doAnimation) {
                        if (gc.getHitbox().intersects(a.getHitbox())) {
                            // 1. Kích hoạt hiệu ứng vỡ hộp
                            gc.setAnimation(true);
                            
                            // 2. Tạo lọ thuốc rơi ra
                            int type = 0;
                            if (gc.getObjType() == BARREL)
                                type = 1;
    ...
    // --- SỬA: TẠO CANNONBALL THAY VÌ PROJECTILE ---
    private void shootCannon(Cannon c) {
        int dir = 1;
        if (c.getObjType() == CANNON_LEFT)
            dir = -1;

        projectiles.add(new CannonBall((int) c.getHitbox().x, (int) c.getHitbox().y, dir));
    }
    ...
    private void drawProjectiles(Graphics g, int xLvlOffset) {
        for (CannonBall p : projectiles)
            if (p.isActive())
                // CannonBall đã có sẵn hàm render, gọi trực tiếp
